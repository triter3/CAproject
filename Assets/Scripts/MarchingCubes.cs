...
// 	}
    ...
    Vector3 MinPoint;
    float Scale;
	public void CreateData(Vector3 min, Vector3 max, float r)
    {
        MinPoint = min;
        Scale = r;
		List<float[]> dims = new List<float[]> ();
		dims.Add (new float[]{ min.x, max.x, r });
		dims.Add (new float[]{ min.y, max.y, r });
		dims.Add (new float[]{ min.z, max.z, r });

		res = new int[3];

		for(var i=0; i<3; ++i) {
			//res[i] = 2 + Mathf.CeilToInt((dims[i][1] - dims[i][0]) / dims[i][2]);
            res[i] = Mathf.CeilToInt((dims[i][1] - dims[i][0]) / dims[i][2]);
		}
		...
		for(int k=0;k<res[2]; ++k, z+=dims[2][2]){
			...
			for (int j = 0; j < res [1]; ++j, y += dims [1] [2]) {
				...
				for (int i = 0; i < res [0]; ++i, x += dims [0] [2], ++n) {
					data[n] = DistanceFunction.Evaluate(new float3(x, y, z));
				}
			}
		}
	}
		...
		for (x [2] = 0; x [2] < dims [2] - 1; ++x [2],n += dims [0]) {
			for (x [1] = 0; x [1] < dims [1] - 1; ++x [1],++n) {
				for (x [0] = 0; x [0] < dims [0] - 1; ++x [0],++n) {
                    ...
                    int auxN = x[0] + dims[0] * (x[1] + dims[1] * x[2]);
					for (int i = 0; i < 8; ++i) {
						Vector3 v = cubeVerts [i];
						//float s = data[Mathf.CeilToInt(n + v[0] + dims[0] * (v[1] + dims[1] * v[2]))];
                        float s = data[Mathf.CeilToInt(auxN + v[0] + dims[0] * (v[1] + dims[1] * v[2]))];
						grid[i]=s;
					...
					for (int i = 0; i < 12; ++i) {
						...
						vertices.Add (nv*Scale + MinPoint);
					}
